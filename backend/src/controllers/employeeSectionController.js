...
export async function listSectionHandler(req, res, next) {
  try {
    ...
    const items = await employeeSectionService.list(req, req.params.section, req.params.id);
    res.json(items);
  } catch (e) {
    next(e);
  }
...
export async function createSectionHandler(req, res, next) {
  try {
    guard(req.params.section);
    const item = await employeeSectionService.create(req, req.params.section, req.params.id, req.body);
...
export async function updateSectionHandler(req, res, next) {
  try {
    ...
    const item = await employeeSectionService.update(
      req, req.params.section, req.params.id, req.params.recordId, req.body
    );
    res.json(item);
  } catch (e) {
    next(e);
...
export async function deleteSectionHandler(req, res, next) {
  try {
    guard(req.params.section);
    await employeeSectionService.remove(req, req.params.section, req.params.id, req.params.recordId);
    res.status(204).end();
  } catch (e) {
    next(e);
  }
