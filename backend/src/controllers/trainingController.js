...
export async function getEnrollmentHandler(req,res,next){ try{ res.json(await service.getEnrollment(req, req.params.id)); }catch(e){next(e);} }
export async function updateEnrollmentHandler(req,res,next){ try{ res.json(await service.updateEnrollment(req, req.params.id, req.body)); }catch(e){next(e);} }
export async function deleteEnrollmentHandler(req,res,next){ try{ await service.deleteEnrollment(req, req.params.id); res.status(204).send(); }catch(e){next(e);} }
