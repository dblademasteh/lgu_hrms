...
- **Recruitment (Hiring Funnel)** — unified `Tabs` surface (Applicants / Interviews / Vacancies). Stage strip (`card p-3`) of compact chip buttons (All + NEW → APPLIED → SCREENED → SHORTLISTED → INTERVIEWED → OFFERED → HIRED + Closed) with mono counts; active chip = `bg-accent/10 text-accent border-accent/25`. **Applicants tab:** filter row (search + position + department selects + count meta) then `lg:grid-cols-[minmax(0,1fr)_360px]` master-detail — left `.data-table` selectable rows (Candidate / Applied for / Department / Stage badge / Joined), right `ApplicantPane` (identity header, vertical stage stepper with Advance/Hire/Reject/Disqualify actions, single-Save score editor, post-hire eligibility list via `listEligibilities`, read-only board notes). **Interviews tab:** upcoming-first table (Applicant / Scheduled / Status badge / Actions); Schedule modal (applicant select, datetime-local, notes). **Vacancies tab:** status filter + table (Title / Plantilla Item / Closes / Status / Actions); New Vacancy modal (plantilla-item select auto-fills title + dept, closesAt, DRAFT/OPEN); Close/Remove with `ConfirmDialog`. Hire modal (appointment type, monthly salary prefill from `PlantillaItem.authorizedSalary`, start date, employee number). All actions toast; destructive actions behind `ConfirmDialog`; empty states in every pane. `/interviews` and `/vacancy` redirect to funnel tabs.
