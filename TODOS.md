...
## Users & Roles reverse-spec (spec-miner pass — Sep 2026, reference: docs/USER_ROLES_SPEC.md)
- [x] **Critical — user creation impossible via the UI (verified):** `GET /users` + `GET /departments` return a **bare array** but 8 call sites read `.data?.items` → `undefined` → `?? []`. Users table was permanently empty and the department `<select>` had only its static `value=""` option, while `Users.jsx:166` requires a truthy `departmentId` — so "Add User" could never submit. Broader damage: department filters/trees empty on Employees, Organization, Appointments, Recruitment, Plantilla + `EmployeeForm`. **Fixed**: normalization moved to the API boundary (`api/shape.js` `toArray`/`listData`; `usersApi.list`/`departmentsApi.list` now resolve to arrays) and all 8 call sites consume the array directly.
- [x] **Critical — every ESS-linked user creation 500'd (verified):** `usersService.create` mixed unchecked scalar FKs (`departmentId`/`tenantId` from `stampTenant`) with a checked relation (`linkedEmployee: { connect }`) in one `prisma.user.create`; Prisma rejects that mix (`Unknown argument 'tenantId'`). Unconditional whenever `externalId` was sent, so `assertLinkable` passed and the write then failed as `INTERNAL_ERROR`. `update` had the same defect via `connect`/`disconnect`. **Fixed**: `externalId` written as a plain scalar (it *is* the FK behind `linkedEmployee`, `schema.prisma:110`). Verified: created a user linked to `EMP-SOLANA-0003`, logged in with the returned `temporaryPassword`, `GET /ess/profile` returned the right employee.
- [x] **Critical — audit trail mis-recorded every successful write (verified):** `audit.js` derived `action` from `req.path` and `entity` from `req.baseUrl`, both of which Express rewrites mid-flight and only restores while unwinding after a throw. Success logged `action: "POST /"` + `entity: "users"`; failure logged `action: "POST /api/v1/users"` + `entity: "root"`. The compliance record could not name the endpoint for any successful mutation, on **every** mutating route, not just users. **Fixed**: both fields now derive from `req.originalUrl` (query stripped) — the only URL Express preserves for the request lifetime. Verified 201 + 400 on `POST /api/v1/users` now log identically.
- [ ] **High — historical AuditLog rows are un-attributable:** rows written before the audit fix keep `entity: "root"` (failures) or a bare `POST /` (successes). Backfill from the request path is impossible after the fact; decide whether to backfill heuristically, annotate the range, or accept the gap before relying on the trail for an incident.
- [ ] **High — `Math.random()` generates account passwords** (`usersService.randomPassword:9`, 12 chars from a 62-symbol set) — not a CSPRNG, for accounts that can reach payroll. Swap to `crypto.randomInt`.
- [ ] **Medium — self-lockout via the permission matrix:** `PATCH /roles/:name/permissions` does not consult `isSystem`, so an admin can strip their own `manageUsersAndRoles` and immediately lose access to the matrix that would restore it (role *deletion* is blocked for system roles, permission edits are not). Needs a last-holder guard.
- [ ] **Medium — `GET /users` is unpaged** while `/employees` is paged; returns all tenant users with two joined relations, which conflicts with the documented ≥1k posture. Add `page`/`limit`.
- [ ] **Medium — layering drift in `routes/roles.js`:** Prisma queries and inline Zod live in the router instead of a `roleService`/`roleRepository` + `shared/contracts/roles.js`, unlike the `/users` path. Contradicts the one-concern-per-file rule in AGENTS.
- [ ] **Low — `passwordChangedAt` is plumbed but never enforced** (set `null` on create); no first-login password rotation. Confirm whether that is intended.
- [ ] **Low — PIN reset is unreachable:** `pinHash` is destructured out of every update path, so a lost PIN cannot be reset through this API. Confirm that is deliberate.
- [ ] **Low — role `name` immutability is silent:** a `name` in a PATCH body is dropped without warning (path param wins). Confirm intended.
- [x] **Docs — reverse-spec written:** `docs/USER_ROLES_SPEC.md` (EARS, every requirement cited to `file:line`); `docs/USER_ROLES_ANALYSIS.md` banner-marked PARTIALLY SUPERSEDED with a discrepancy table, since it predates the capability matrix (still claims `requireRole()` is the gate, passwords are `"changeme"`, and role changes are unaudited — all false).
...
- [x] **High — "Standard Reports" is a stub catalogue** → REPLACED: 4 hardcoded `data/reports.js` entries (now removed) replaced with 3 real templates backed by `/reports/payroll-register`, `/reports/payroll-journal`, `/reports/employee-master-list` endpoints; Generate=toast stub replaced with real CSV download; Preview modal removed; "3 templates" label fixed to "{n} templates" — verified live: `npm run build` passes, color lint clean
- [x] **High — only ONE backend report endpoint** → FIXED: added `GET /reports/payroll-register` (per-employee rows + per-code deduction columns), `GET /reports/payroll-journal` (LedgerEntry-derived debit/credit CSV), `GET /reports/employee-master-list` (personnel directory CSV); all return tenant-scoped CSV with UTF-8 BOM; remittances/DTR/Service Record still P2/P3
- [x] **High — summary shows unposted runs** (`reportsController.js:28-33`) → FIXED: periodId and default lookups now filter `status: 'POSTED'` (unposted runs excluded, matching ESS behavior)
- [x] **Medium — export hygiene:** CSV downloads now include UTF-8 BOM (DB table export + new reports); money values use 2dp; `downloadCsvBlob` helper in Reports.jsx handles content-disposition filenames. DB `exportData` now handles Date/JSON/Buffer types correctly (was `String(val)` on everything)
- [ ] **Medium — 5+ duplicated client export blobs** (Audit/Employees/Disqualifications/Attendance) — shared `lib/export.js` (`downloadCsv` w/ quoting + Manila filename) then adopt
- [ ] **P2/P3 — server-side report engine:** remittance groups by `PayrollDeductionLine.code` (GSIS/Pag-IBIG/Philhealth/BIR 1601-C), DTR report, leave/loan extracts, Service Record PDF — then install pdfmake/ExcelJS (or server render) for those; payroll-register/journal/employee-master-list now done
