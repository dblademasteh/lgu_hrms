...
- `/learning` — **Learning & Development** (`frontend/src/pages/Learning.jsx`, training programs + enrollments).
...
- `backend/src/routes/training.js` — full router behind `requirePermission('trainingCRUD')`: /programs CRUD + /enrollments GET list + POST only (**no enrollment PATCH/DELETE**).
...
### Enrollments
- Create only (ENROLLED default). Status enum gaps **by design**: DB `EnrollmentStatus={ENROLLED,COMPLETED,CANCELLED}`; contract also allows `IN_PROGRESS` → **HTTP 500 crash** (verified). No transition wiring, `completedAt` never set.
